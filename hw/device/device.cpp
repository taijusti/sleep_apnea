...
static void init_device(data_t data [ELEMENTS], hls::stream<uint32_t> * in,
        bool y [ELEMENTS], uint32_t e_bram [ELEMENTS], uint32_t alpha [ELEMENTS]) {
    int i, j;
...
static void kkt_pipeline (data_t * point0, data_t * point1, data_t data [ELEMENTS],
        uint32_t e_bram[ELEMENTS], uint32_t alpha [ELEMENTS],
        bool y [ELEMENTS],  hls::stream<uint32_t> * kkt_bram_fifo, uint32_t * kkt_violators,
        uint32_t y1_delta_alpha1_product, uint32_t y2_delta_alpha2_product, uint32_t delta_b,
        uint32_t idx) {
#pragma HLS DATAFLOW

    hls::stream<uint32_t> k1_fifo;
    #pragma HLS STREAM variable=k1_fifo depth=64
    hls::stream<uint32_t> k2_fifo;
    #pragma HLS STREAM variable=k2_fifo depth=64
    hls::stream<uint32_t> e_fifo;
    #pragma HLS STREAM variable=e_fifo depth=64
    hls::stream<data_t> data_fifo;
    #pragma HLS STREAM variable=data_fifo depth=64
    hls::stream<uint32_t> alpha_fifo;
    #pragma HLS STREAM variable=alpha_fifo depth=64
    hls::stream<bool> y_fifo;
    #pragma HLS STREAM variable=y_fifo depth=64
    hls::stream<uint32_t> e_bram_fifo;
    #pragma HLS STREAM variable=e_bram_fifo depth=64
    ...
    e(&e_bram_fifo, &e_fifo, &k1_fifo, &k2_fifo,
...
void device(hls::stream<uint32_t> * in, hls::stream<uint32_t> * out) {
    ...
    static uint32_t y1_delta_alpha1_product;
    static uint32_t y2_delta_alpha2_product;
    static uint32_t delta_b;
    static uint32_t target_e;
