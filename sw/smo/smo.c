...
int classify_2(DATA * x, int * alpha, int b, int numData, int numDim, int classified_point)
{
	int i;
	int sum = 0;
	for (i = 0; i < numData; i++)
...
bool checkKKT(int alpha, int yuProduct) {
...
void smo(DATA * data, CLASSIFIER * classifier, int numDim, int numData) {
	...
	while (1) {
		...
		// that violates KKT
		for (i = 0; i < numData; i++) {
			yuProduct = data[i].y * classify_2(data, alpha, b, numData, numDim, i);

			if (checkKKT(alpha[i], yuProduct) == false) {
				d1Idx = i;
				break;
			}
		}
		...
		err[d1Idx] = classify_2(data, alpha, b, numData, numDim, d1Idx) - data[d1Idx].y;
		err[d2Idx] = classify_2(data, alpha, b, numData, numDim, d2Idx) - data[d2Idx].y;
		...
		// update calculate B
		b1 = err[d1Idx] + (data[d1Idx].y * (alpha1New - alpha[d1Idx]) * k11) + (data[d2Idx].y * (alpha2NewClipped - alpha[d2Idx]) * k12) + b;
		b2 = err[d2Idx] + (data[d1Idx].y * (alpha1New - alpha[d1Idx]) * k12) + (data[d2Idx].y * (alpha2NewClipped - alpha[d2Idx]) * k22) + b;
