...
#define C (1000) // hyperparameter for Lagrange multiplier. all alpha must obey 0 <= alpha <= C
...
int smoChooseSecond(int d2Idx)
{
	...
	for (i = 0; i < numData; i++) {
		...
		if ((ABS(err - err2) > maxErr)&&(i!=d2Idx)) {
			err1 = err;
